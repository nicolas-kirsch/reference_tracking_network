...
save_folder = os.path.join(
    save_path,
    f"perf_boost_{now}_dim_nl_{args.dim_nl}_dim_internal_{args.dim_internal}_num_rollouts_{args.num_rollouts}"
)
...
# xbar_train = torch.tensor([2, 3, 0, 0, -2., 3 ,0 ,0 ])
...
# data_verif[3:4, 1:data_verif.size(1)//2, 8:] = xbar_train
...
#     x_verif[3, :, :], # remove extra dim due to batching
#     xbar=xbar_train, n_agents=sys.n_agents,
#     save_folder=save_folder, filename='CL_round_trip_trained.pdf',
#     text="CL - round trip - Trained", T=t_ext, 
#     obstacle_centers=loss_fn.obstacle_centers,
#     obstacle_covs=loss_fn.obstacle_covs, round_trip=True, xbar2=dataset.x0
# )
