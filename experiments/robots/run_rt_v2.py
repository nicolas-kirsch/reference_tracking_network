...
# if args.rt_epochs > 0:
#     back_valid_data = back_train_data
...
logger.info('\n[INFO] evaluating the FORWARD trained controller on %i test rollouts.' % test_data.shape[0])
with torch.no_grad():
    # simulate over horizon steps
    x_log, e_log, u_log= sys.rollout(
...
# -------- 8. Backward training --------
if args.rt_epochs > 0:
    # Initialize the backward dataset
    x0_back = x_verif[0, -1 ,:4*args.n_agents].detach()
    xbar_back = x_verif[0, 0 ,:4*args.n_agents].detach()
    back_dataset = RobotsDataset(random_seed=args.random_seed, horizon=args.horizon,
                                         x_bar=xbar_direct, x0=x0_back, std_ini=args.std_init_plant, n_agents=args.n_agents)

    back_train_data, back_test_data = back_dataset.get_data(num_train_samples=args.num_rollouts,
                                                             num_test_samples=500, y_interval=(0, 0.1))
    back_train_data, back_test_data = back_train_data.to(device), back_test_data.to(device)
    back_train_dataloader = DataLoader(back_train_data, batch_size=args.batch_size, shuffle=True)
    back_valid_data = back_train_data

    # Initialize the backward controller
    ...
    # Initialize the backward loss function
    loss_fn_back = RobotsLoss(
        Q=Q,Qs = Qs, alpha_u=args.alpha_u, xbar=back_train_data[0,:,8:],
        loss_bound=None, sat_bound=None,
        alpha_col=args.alpha_col, alpha_obst=args.alpha_obst,obstacle_centers=obstacle_centers,obstacle_covs=obstacle_covs,
    ...
    # Initialize the backward optimizer
    back_optimizer = torch.optim.Adam(ctl_back.parameters(), lr=args.lr)

    # CSet the parameters of the backward controller to the best parameters of the forward controller
    ...
    # Evaluate the backward controller
    logger.info('\n[INFO] evaluating the BACKWARD trained controller on %i test rollouts.' % back_test_data.shape[0])
    
    with torch.no_grad():
        # simulate over horizon steps
        x_log, e_log, u_log= sys.rollout(
            controller=ctl_back, data=back_test_data, train=False,
        )
        test_metrics = compute_distance_metric(x_log, back_test_data, sys.n_agents)
        # loss
        test_loss, test_obst_loss = loss_fn_back.forward(x_log, u_log,e_log)[:2]
        test_loss, test_obst_loss = test_loss.item(), test_obst_loss.item()
    ...
    test_collisions = 0
    if args.col_av:
        test_collisions, percentage_test_collisions = loss_fn_back.count_collisions(x_log)
        msg += ' -- Number of collisions = %i' % test_collisions + ' -- Percentage of collisions = %.2f' % percentage_test_collisions
    if args.alpha_obst:
        obst_col, obst_col_percentage, obst_col_cases = loss_fn_back.count_obstacle_collisions(x_log, test_data)
        msg += ' -- Number of obstacle collisions = %i' % obst_col + ' -- Percentage of obstacle collisions = %.2f' % obst_col_percentage
    logger.info(msg)
    # Do a backward rollout
    data_verif_back = torch.zeros(1, args.horizon+200, 16)
    data_verif_back[:, 0:1, :8] = \
        back_dataset.x0
    data_verif_back[0:1, 1:, 8:] = \
        xbar_back
    
    x_verif, e_verif, u_verif = sys.rollout(ctl_back, data_verif_back)
    v_verif = sys.v_log
    distance_metric_back = compute_distance_metric(x_verif, data_verif_back, sys.n_agents)
    msg = "Distance metric Back: %.4f" % (distance_metric_back)
    logger.info(msg)

    plot_trajectories(
        x_verif[0, :, :], # remove extra dim due to batching
        xbar=xbar_back, n_agents=sys.n_agents,
        save_folder=save_folder, filename='CL_diag_trained_back.png',
        text="rPB - trained controller", T=t_ext, 
        obstacle_centers=loss_fn.obstacle_centers,
        obstacle_covs=loss_fn.obstacle_covs
    )
...
plot_trajectories(
    x_verif[0, :, :], # remove extra dim due to batching
    xbar=xbar_direct, n_agents=sys.n_agents,
    save_folder=save_folder, filename='CL_diag_trained_rt.png',
    text="rPB - trained controller", T=t_ext, 
    obstacle_centers=loss_fn.obstacle_centers,
    obstacle_covs=loss_fn.obstacle_covs, save= not args.no_save_plot
)
