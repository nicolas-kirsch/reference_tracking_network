...
save_path = os.path.join(BASE_DIR, 'experiments', 'robots', 'saved_results_rt_V7')
...
if args.rt_epochs > 0:
    ...
    back_train_data, back_test_data = back_dataset.get_data(num_train_samples=args.num_rollouts, num_test_samples=500)
...
# Plot the round-trip trajectory
# plot_trajectories(
...
if args.model_path:
    ...
    for epoch in range(1+args.epochs):
        ...
        for train_data_batch in train_dataloader:
            ...
            # If within the range of `args.rt_epochs`, add the backward rollout
            # if epoch >= args.epochs * (1 - args.rt_epochs):
        ...
        if epoch%args.log_epoch == 0:
            ...
            if args.return_best:
                ...
                with torch.no_grad():
                    ...
                    # if epoch >= args.epochs * (1 - args.rt_epochs):
                        # Augment the data with the backward rollout
                        # x_log_valid, e_log_valid, u_log_valid = sys.augmented_rollout(controller=ctl, data=valid_data, train=False,
                        #                                                 x_log=x_log_valid, e_log=e_log_valid, u_log=u_log_valid)
        ...
        for epoch in range(1+int(args.rt_epochs*args.epochs)):
            ...
            if epoch%args.log_epoch == 0:
                ...
                if args.return_best:
                    ...
                    with torch.no_grad():
                        x_log_valid, e_log_valid, u_log_valid = sys.rollout(
                            controller=ctl, data=back_train_data, train=False,
                        )
                    ...
                    if loss_valid.item()<best_valid_loss:
                        best_valid_loss = loss_valid.item()
                        best_params_ren = ctl.get_parameters_as_vector()
                        best_params_mlp = ctl.get_mlp_parameters()
                        msg += ' (best so far)'
                duration = time.time() - t
                msg += ' ---||--- time: %.0f s' % (duration)
