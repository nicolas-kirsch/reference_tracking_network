...
# dataset2 = RobotsDataset(random_seed=args.random_seed, horizon=args.horizon, x_bar=xbar_diag, x0=x0_2, std_ini=args.std_init_plant, n_agents=args.n_agents)
...
ctl2 = PerfBoostController(
    noiseless_forward=sys2.noiseless_forward,
    input_init=sys2.x_init, output_init=sys2.u_init,
    dim_internal=args.dim_internal, dim_nl=args.dim_nl,
    initialization_std=args.cont_init_std,
    output_amplification=20,
).to(device)
...
loss_fn_2 = RobotsLoss(
    ...
    alpha_col=args.alpha_col, alpha_obst=args.alpha_obst,obstacle_centers=obstacle_centers,obstacle_covs=obstacle_covs,
    min_dist=args.min_dist if args.col_av else None,
    n_agents=sys2.n_agents if args.col_av else None,
)
