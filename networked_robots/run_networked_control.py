...
def build_complete_graph_adjacency(n_agents):
    ...
    directly to RobotsNetwork/PBControllerNetwork. No CLI topology flag for
    this pass: the caller constructs the matrix explicitly.
    """
    return torch.ones(n_agents, n_agents) - torch.eye(n_agents)


def format_loss_components(parts):
    """'tracking: 1.2345 -- energy: 0.0100' from a NetworkedRobotsLoss.components() dict."""
...
def plot_rollout_diagnostics(net, controller, plot_data, args, save_folder, tag, text_prefix):
    """
    Roll out `plot_data` under `controller` (and, for comparison, under the
    base loop alone), and save trajectory + reference-offset-norm plots for
    the first `args.plot_samples` rollouts. Returns the mean final-timestep
    tracking error (a quick scalar summary).
    """
    x_log, e_log, u_log = net.rollout(plot_data, controller=controller)
    x_log_base, _, _ = net.rollout(plot_data)

    xbar_cols = [4 * i + j for i in range(args.n_agents) for j in (0, 1)]
    xbar_full = plot_data[:, :, net.state_dim:2 * net.state_dim][:, :, xbar_cols]
    ...
    for r in range(num_plots):
        plot_trajectories(
            ...
            filename=f'{tag}_rollout_{r}.pdf',
            text=f'{text_prefix} - rollout {r}',
            T=args.horizon,
            x_compare=x_log_base[r, :, :],
        ...
        plot_input_norm(
            ...
            n_agents=args.n_agents,
            save_folder=save_folder,
            filename=f'{tag}_dxref_norm_{r}.pdf',
            text=f'Reference offset $\\delta x_{{ref}}$ norm per robot ({tag}) - rollout {r}',
            T=args.horizon,
        )

    return e_log[:, -1, :].abs().mean().item()


def main():
    now = datetime.now().strftime("%m_%d_%H_%M_%S")
    save_path = os.path.join(BASE_DIR, 'networked_robots', 'saved_results')
    save_folder = os.path.join(save_path, 'networked_control_' + now)
    ...
    logger = logging.getLogger('networked_robots_networked_control')
